const axios = $http
const sckey = $prefs.get("SCKEY");
const qmsgkey = $prefs.get("qmsgapi");
const cpkey = $prefs.get("cpapi");
const pushplustoken = $prefs.get("pushplustoken")
const wxtoken =$prefs.get("wx").split("@")
const corpsecret = wxtoken[0]
const corpid = wxtoken[1]
const agentid =wxtoken[2]
const mediaid=wxtoken[3]//素材库图片id
const tg =$prefs.get("tg").split("@")
const tgbotoken=tg[0] //bot token
const chatid=tg[1] //私聊就是userid 群聊就是群组id 可通过userinfo机器人获得
...
function wx(msg) {
    return new Promise(async (resolve) => {
       ...
       let data =mediaid!=0?mpnews:text
